...
def CP77SubPrep(self, context, smooth_factor, merge_distance):
    ...
    bpy.ops.mesh.select_non_manifold(
        extend=False, use_wire=True, use_boundary=True, use_multi_face=False, use_non_contiguous=False, use_verts=False
    )
    ...
    with zipfile.ZipFile(target_body_path, "r") as z:
        ...
        if data:
            ...
            if fbx_rot:
                ...
                new_lattice.rotation_euler = (
                    data["lattice_object_rotation"][0],
                    data["lattice_object_rotation"][1],
                    data["lattice_object_rotation"][2] + 3.14159,
                )
            else:
                new_lattice.rotation_euler = (
                    data["lattice_object_rotation"][0],
                    data["lattice_object_rotation"][1],
                    data["lattice_object_rotation"][2],
                )
            ...
            flat_lattice_points = [
                lattice_points[w + v * lattice.points_u + u * lattice.points_u * lattice.points_v]
