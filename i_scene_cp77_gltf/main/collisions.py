...
def CP77CollisionGen(
    self, context, matchSize, collider_type, collision_shape, sampleverts, radius, height, physics_material
):
    is_edit_mode = bpy.context.object.mode == "EDIT"
        ...
        if collision_shape == "CONVEX":
            ...
            if len(selected_verts) < verts_to_sample:
                ...
                bpy.ops.cp77.message_box(
                    "INVOKE_DEFAULT", message="Sample number is higher than selected vertices count!"
                )
                return {"CANCELLED"}
            else:
                # sample the vertices
                step_size = len(selected_verts) // verts_to_sample
                sampled_verts = [selected_verts[i] for i in range(0, len(selected_verts), step_size)][:verts_to_sample]
