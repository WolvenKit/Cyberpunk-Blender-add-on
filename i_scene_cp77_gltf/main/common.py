...
def json_ver_validate(json_data):
    if "Header" not in json_data.keys():
        return False
    elif "MaterialJsonVersion" in json_data["Header"].keys() and pkg_resources.parse_version(
        json_data["Header"]["MaterialJsonVersion"]
    ) > pkg_resources.parse_version("1.0.0RC"):
        return True
...
def CreateShaderNodeRGB(curMat, color, x=0, y=0, name=None, isVector=False):
    ...
    if isVector:
        ...
        rgbNode.outputs[0].default_value = (
            float(color["Red"]) / 255,
...
def createOverrideTable(matTemplateObj):
    ...
    for x in OverList["roughLevelsOut"]:
        ...
        Output["RoughLevelsOut"][tmpName] = [
            (tmpStrength0, tmpStrength0, tmpStrength0, 1),
            (tmpStrength1, tmpStrength1, tmpStrength1, 1),
        ]
...
def createParallaxGroup():
    if "CP77_Parallax" in bpy.data.node_groups.keys():
        ...
        Tangent = create_node(CurMat.nodes, "ShaderNodeTangent", (-565.0, -136.0), label="Tangent")
        Tangent.direction_type = "UV_MAP"
        VectorMath = create_node(
            CurMat.nodes, "ShaderNodeVectorMath", (-566.0, -342.0), operation="CROSS_PRODUCT", label="Vector Math"
        )
        VectorMath002 = create_node(
            CurMat.nodes, "ShaderNodeVectorMath", (-227.0, -208.0), operation="DOT_PRODUCT", label="Vector Math.002"
        )
        VectorMath004 = create_node(
            CurMat.nodes, "ShaderNodeVectorMath", (361.0, 34.0), operation="SCALE", label="Vector Math.004"
        )
        VectorMath005 = create_node(
            CurMat.nodes, "ShaderNodeVectorMath", (581.0, 123.0), operation="SUBTRACT", label="Vector Math.005"
        )
        UVMap = create_node(CurMat.nodes, "ShaderNodeUVMap", (299.0, 342.0), label="UV Map")
        VectorMath001 = create_node(
            CurMat.nodes, "ShaderNodeVectorMath", (-248.0, 37.0), operation="DOT_PRODUCT", label="Vector Math.001"
        )
        VectorMath006 = create_node(
            CurMat.nodes, "ShaderNodeVectorMath", (-95.0, 332.0), operation="DOT_PRODUCT", label="Vector Math.006"
        )
        Geometry = create_node(CurMat.nodes, "ShaderNodeNewGeometry", (-581.0, 222.0), label="Geometry")
        Math = create_node(CurMat.nodes, "ShaderNodeMath", (159.0, -230.0), operation="DIVIDE", label="Math")
        CombineXYZ = create_node(CurMat.nodes, "ShaderNodeCombineXYZ", (-13.0, 31.0), label="Combine XYZ")
        GroupInput = create_node(CurMat.nodes, "NodeGroupInput", (-781.0, 0.0), label="Group Input")
...
def CreateGradMapRamp(CurMat, grad_image_node, location=(-400, 250)):
    ...
    for x in range(image_width):
        pixel_data = grad_image_node.image.pixels[
            (row_index * image_width + x) * 4 : (row_index * image_width + x) * 4 + 3
        ]
        color = Color()
...
def createHash12Group():
    if "hash12" in bpy.data.node_groups.keys():
        ...
        vecMul.inputs[1].default_value = (0.1031, 0.1031, 0.1031)
