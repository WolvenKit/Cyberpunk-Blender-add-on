...
def importSectors(filepath="", want_collisions=False, am_modding=False, with_materials=True):
            ...
            match type:
                case "worldEntityNode" | "worldDeviceNode":
                    ...
                    if meshname != 0:
                        meshes.append(
                            {
                                "basename": e["Data"]["entityTemplate"]["DepotPath"]["$value"],
                                "appearance": e["Data"]["appearanceName"],
                                "sector": sectorName,
                            }
                        )
                case "worldInstancedMeshNode":
                    ...
                    if meshname != 0:
                        meshes.append(
                            {
                                "basename": data["mesh"]["DepotPath"]["$value"],
                                "appearance": e["Data"]["meshAppearance"],
                                "sector": sectorName,
                            }
                        )
                case (
            ...
            if groupname not in Masters.children.keys() and os.path.exists(meshpath):
                try:
                    bpy.ops.io_scene_gltf.cp77(
                        filepath=meshpath, appearances=impapps, update_gi=False, with_materials=with_materials
                    )
    ...
    inst_pos = (0, 0, 0)
    inst_rot = Quaternion((0.707, 0, 0.707, 0))
                    ...
                    case "worldEntityNode" | "worldDeviceNode":
                        ...
                        if ent_groupname in Masters.children.keys():
                            ...
                            try:
                                ...
                                bpy.ops.io_scene_gltf.cp77entity(
                                    filepath=entpath, appearances=app, with_materials=with_materials, inColl=incoll
                                )
                                move_coll = Masters.children.get(ent_groupname)
                                imported = True
                            except:
                        ...
                        if imported:
                            ...
                            for idx, inst in enumerate(instances):
                                ...
                                if group:
                                    ...
                                    new["nodeType"] = type
                                    new["nodeIndex"] = i
                                    new["nodeDataIndex"] = next(
                                        (index for index, obj in enumerate(t) if obj["NodeIndex"] == i), None
                                    )
                                    new["instance_idx"] = idx
                                    new["debugName"] = e["Data"]["debugName"]
                                    new["sectorName"] = sectorName
                                    new["HandleId"] = e["HandleId"]
                                    new["entityTemplate"] = os.path.basename(
                    ...
                    case "worldBendedMeshNode" | "worldCableMeshNode":
                        ...
                        if len(instances) > 0 and (meshname != 0):
                            ...
                            for idx, tt in enumerate(defData):
                                M = Matrix(
                                    (
                                        ext_row(defData[idx]["X"]),
                                        ext_row(defData[idx]["Y"]),
                                        ext_row(defData[idx]["Z"]),
                                        ext_row(defData[idx]["W"]),
                                    )
                                )
                                M = M.transposed()
                                ...
                                for idx in range(start, start + num):
                                    ...
                                    for old_obj in group.all_objects:
                                        ...
                                        if "Data" in data["worldTransformsBuffer"]["sharedDataBuffer"].keys():
                                            inst_trans = data["worldTransformsBuffer"]["sharedDataBuffer"]["Data"][
                                                "buffer"
                                            ]["Data"]["Transforms"][idx]

                                        elif "HandleRefId" in data["worldTransformsBuffer"]["sharedDataBuffer"].keys():
                                            bufferID = int(
                                                data["worldTransformsBuffer"]["sharedDataBuffer"]["HandleRefId"]
                                            )
                                            ...
                                            for n in nodes:
                                                if n["HandleId"] == str(bufferID - 1):
                                                    ref = n
                                            inst_trans = ref["Data"]["worldTransformsBuffer"]["sharedDataBuffer"][
                                                "Data"
                                            ]["buffer"]["Data"]["Transforms"][idx]
                                        else:
                                            print(e)
                                    ...
                                    for inst in instances:
                                        ...
                                        for old_obj in group.all_objects:
                                            ...
                                            bpy.context.scene.cursor.location = Vector(
                                                (
                                                    inst["Pivot"]["X"] / scale_factor,
                                                    inst["Pivot"]["Y"] / scale_factor,
                                                    inst["Pivot"]["Z"] / scale_factor,
                                                )
                                            )
                                    ...
                                    for idx, inst in enumerate(instances):
                                        ...
                                        for old_obj in group.all_objects:
                                            ...
                                            if type == "worldRotatingMeshNode":
                                                ...
                                                obj.rotation_euler[axis_no] = obj.rotation_euler[
                                                    axis_no
                                                ] + math.radians(360)
                                                obj.keyframe_insert(
                                                    "rotation_euler", index=axis_no, frame=rot_time * 24
                                                )
                                    ...
                                    for instidx, inst in enumerate(instances):
                                        ...
                                        for idx in range(start, start + num):
                                            ...
                                            if "Data" in data["cookedInstanceTransforms"]["sharedDataBuffer"].keys():
                                                ...
                                                inst_trans = data["cookedInstanceTransforms"]["sharedDataBuffer"][
                                                    "Data"
                                                ]["buffer"]["Data"]["Transforms"][idx]

                                            elif (
                                                "HandleRefId"
                                                in data["cookedInstanceTransforms"]["sharedDataBuffer"].keys()
                                            ):
                                                bufferID = int(
                                                    data["cookedInstanceTransforms"]["sharedDataBuffer"]["HandleRefId"]
                                                )
                                                new["bufferID"] = bufferID
                                                ref = e
                                                for n in nodes:
                                                    if n["HandleId"] == str(bufferID - 1):
                                                        ref = n
                                                inst_trans = ref["Data"]["cookedInstanceTransforms"][
                                                    "sharedDataBuffer"
                                                ]["Data"]["buffer"]["Data"]["Transforms"][idx]

                                            else:
                                                print(e)

                                            inst_trans_rot = Quaternion(
                                                (
                                                    inst_trans["orientation"]["r"],
                                                    inst_trans["orientation"]["i"],
                                                    inst_trans["orientation"]["j"],
                                                    inst_trans["orientation"]["k"],
                                                )
                                            )
                                            ...
                                            inst_scale = Vector((1, 1, 1))
                                            inst_trans_m = Matrix.LocRotScale(
                                                inst_trans_pos, inst_trans_rot, inst_trans_scale
                                            )
                                            inst_m = Matrix.LocRotScale(inst_pos, inst_rot, inst_scale)
