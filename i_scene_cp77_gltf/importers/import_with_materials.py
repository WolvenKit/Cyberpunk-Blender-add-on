...
def objs_in_col(top_coll, objtype):
    return sum([len([o for o in col.objects if o.type == objtype]) for col in top_coll.children_recursive]) + len(
        [o for o in top_coll.objects if o.type == objtype]
    )
    ...
    for f in loadfiles:
        ...
        if with_materials and os.path.exists(BasePath + ".Material.json"):
            ...
            if not valid_json:
                self.report(
                    {"ERROR"},
                    "Incompatible material.json file detected. This add-on version requires materials generated WolvenKit 8.9.1 or higher.",
                )
                break
                        ...
                        for matname in gltf_importer.data.meshes[counter].extras["materialNames"]:
                            if matname in validmats.keys():
                                ...
                                if (
                                    ...
                                    and "glass" not in matname
                                    and "MaterialTemplate" not in matname
                                    and "Window" not in matname
                                    and matname[:5] != "Atlas"
                                    and "BaseMaterial" in bpy_mats[matname].keys()
                                    and bpy_mats[matname]["BaseMaterial"] == m["BaseMaterial"]
                                    and bpy_mats[matname]["GlobalNormal"] == m["GlobalNormal"]
                                    and bpy_mats[matname]["MultilayerMask"] == m["MultilayerMask"]
                                ):
                                    ...
                                    and matname[:5] == "Atlas"
                                    and bpy_mats[matname]["BaseMaterial"] == m["BaseMaterial"]
                                    and bpy_mats[matname]["DiffuseMap"] == m["DiffuseMap"]
                                ):
                                    bpy.data.meshes[name].materials.append(bpy_mats[matname])
                                else:
                                    if matname in validmats.keys():
