...
def importEnt(
    ...
    if not valid_json:
        bpy.ops.cp77.message_box(
            "INVOKE_DEFAULT",
            message="Incompatible entity json file detected. This add-on version requires materials generated WolvenKit 8.9.1 or higher.",
        )
        return {"CANCELLED"}
            ...
            for anim in anim_files:
                if os.path.exists(anim[:-3] + "anims.json"):
                    ...
                    if not valid_json:
                        bpy.ops.cp77.message_box(
                            "INVOKE_DEFAULT",
                            message="Incompatible anim json file detected. This add-on version requires materials generated WolvenKit 8.9.1 or higher.",
                        )
                        return {"CANCELLED"}
        ...
        if len(entrigjsons) > 0:
            with open(entrigjsons[0], "r") as f:
                ...
                if not valid_json:
                    bpy.ops.cp77.message_box(
                        "INVOKE_DEFAULT",
                        message="Incompatible rig json file detected. This add-on version requires materials generated WolvenKit 8.9.1 or higher.",
                    )
                    return {"CANCELLED"}
            ...
            if len(ent_apps) > 0:
                ...
                if os.path.exists(appfilepath):
                    ...
                    if not valid_json:
                        bpy.ops.cp77.message_box(
                            "INVOKE_DEFAULT",
                            message="Incompatible app json file detected. This add-on version requires materials generated WolvenKit 8.9.1 or higher.",
                        )
                        return {"CANCELLED"}
                    ...
                    if "Data" in a_j["Data"]["RootChunk"]["appearances"][app_idx].keys():
                        ...
                        if "compiledData" in a_j["Data"]["RootChunk"]["appearances"][app_idx]["Data"].keys():
                            chunks = a_j["Data"]["RootChunk"]["appearances"][app_idx]["Data"]["compiledData"]["Data"][
                                "Chunks"
                            ]
                            print("Chunks found")
                else:
                            ...
                            if os.path.exists(meshpath):
                                ...
                                try:
                                    ...
                                    if pT:
                                        ...
                                        if chunks:
                                            for chunk in chunks:
                                                if "parentTransform" in chunk.keys() and isinstance(
                                                    chunk["parentTransform"], dict
                                                ):
                                                    # print('pt found')
                                        ...
                                        if chunk_pt:
                                            ...
                                            elif bindname == "deformation_rig" and (
                                                not chunk_pt["Data"]["slotName"]["$value"]
                                                or len(chunk_pt["Data"]["slotName"]["$value"]) == 1
                                            ):
                                                json_name = os.path.join(
                                                    path, c["mesh"]["DepotPath"]["$value"] + ".json"
                                                )
                                                    ...
                                                    if "boneRigMatrices" in mesh_j.keys():
                                                        ...
                                                        for obj in objs:
                                                            ...
                                                            bpy.ops.constraint.childof_set_inverse(
                                                                constraint="Child Of", owner="OBJECT"
                                                            )
                                            ...
                                            # things like the tvs have a bindname but no slotname, bindname appears to point at the name of the main component, and its the only one with a transform applied
                                                ...
                                                if bindpt and len(bindpt) == 1:
                                                    if (
                                                        c["localTransform"]["Position"]["x"]["Bits"] == 0
                                                        and c["localTransform"]["Position"]["y"]["Bits"] == 0
                                                        and c["localTransform"]["Position"]["z"]["Bits"] == 0
                                                    ):
                                                        c["localTransform"]["Position"] = bindpt[0]["localTransform"][
                                                            "Position"
                                                        ]
                                                    if (
                                                        c["localTransform"]["Orientation"]["i"] == 0
                                                        and c["localTransform"]["Orientation"]["j"] == 0
                                                        and c["localTransform"]["Orientation"]["k"] == 0
                                                        and c["localTransform"]["Orientation"]["r"] == 1
                                                    ):
                                                        c["localTransform"]["Orientation"] = bindpt[0][
                                                            "localTransform"
                                                        ]["Orientation"]
                                                        ...
                                                        if o["name"]["$value"] == bindname:
                                                            ...
                                                            for chunk in chunks:
                                                                if "parentTransform" in chunk.keys() and isinstance(
                                                                    ...
                                                                    if "HandleId" in chunk["parentTransform"].keys():
                                                                        if (
                                                                            chunk["parentTransform"]["HandleId"]
                                                                            == pT_HId
                                                                        ):
                                                                            chunk_pt = chunk["parentTransform"]
                                                                            # print('HandleId found',chunk['parentTransform']['HandleId'])
                                    ...
                                    license_plates = [
                                        ...
                                        if "license_plate" in obj.get("componentName", "")
                                    ]
                                    bumper_f_objs = [
                                        obj for obj in bpy.data.objects if "bumper_f" in obj.get("componentName", "")
                                    ]
                                    bumper_b_objs = [
                                        obj for obj in bpy.data.objects if "bumper_b" in obj.get("componentName", "")
                                    ]
                                    ...
                                    if len(license_plates) > 0:
                                        for obj in license_plates:
                                            try:
                                                ...
                                                bumper_type = (
                                                    "bumper_f" if "license_plate_f" in componentName else "bumper_b"
                                                )
                                                # Find the correct bumper and match it to the license plate
                                                potential_parents = (
                                                    bumper_f_objs if bumper_type == "bumper_f" else bumper_b_objs
                                                )
                                                ...
                                                if potential_parents:
                                                    ...
                                                    lct = next(
                                                        (
                                                            comp
                                                            for comp in comps
                                                            if comp["name"]["$value"] == componentName
                                                        ),
                                                        None,
                                                    )
                                                    ...
                                                    if lct:
                                                        obj.location[0] = (
                                                            lct["localTransform"]["Position"]["x"]["Bits"] / 131072
                                                        )
                                                        obj.location[1] = (
                                                            lct["localTransform"]["Position"]["y"]["Bits"] / 131072
                                                        )
                                                        obj.location[2] = (
                                                            lct["localTransform"]["Position"]["z"]["Bits"] / 131072
                                                        )
                                                else:
                ...
                try:
                    ...
                    if len(physJsonPaths) == 0:
                        ...
                        if len(chassis_info) > 0:
                            ...
                            chassis_phys_j = (
                                os.path.basename(chassis_info["collisionResource"]["DepotPath"]["$value"]) + ".json"
                            )
                        else:
                        ...
                        if i["$type"] == "entColliderComponent":
                            ...
                            if collision_shape == "physicsColliderBox":
                                try:
                                    ...
                                    draw_box_collider(
                                        submeshName, new_col, half_extents, transform, physmat, collision_type
                                    )
                            ...
                            if collision_shape == "physicsColliderConvex":
                                try:
                                    ...
                                    obj = draw_convex_collider(
                                        submeshName, new_col, vertices, transform, physmat, collision_type
                                    )
                                    convcol = obj
                            ...
                            if collision_shape == "physicsColliderSphere":
                                try:
                                    r = cdata["radius"]
                                    submeshName = "_" + collision_shape
                                    obj = draw_sphere_collider(
                                        submeshName, new_col, r, position, physmat, collision_type
                                    )
                                except Exception as e:
                                    print("uh oh", e)
                            if collision_shape == "physicsColliderCapsule":
                                try:
                                    ...
                                    h = cdata["height"]
                                    submeshName = "_" + collision_shape
                                    obj = draw_capsule_collider(
                                        submeshName, new_col, r, h, position, rotation, physmat, collision_type
                                    )
                                except Exception as e:
                                    print("uh oh", e)
