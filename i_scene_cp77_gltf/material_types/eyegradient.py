...
class EyeGradient:
    ...
    def create(self, Data, Mat):
        ...
        if not valid_json:
            self.report(
                {"ERROR"},
                "Incompatible eye gradient json file detected. This add-on version requires materials generated WolvenKit 8.9.1 or higher.",
            )
            return
        ...
        if "Normal" in Data:
            nMap = CreateShaderNodeNormalMap(
                CurMat, self.BasePath + Data["Normal"], -150, -250, "Normal", self.image_format
            )
        ...
        # ALBEDO/a
        if "Albedo" in Data:
            aImgNode = CreateShaderNodeTexImage(
                CurMat, self.BasePath + Data["Albedo"], -550, 150, "Albedo", self.image_format
            )

        # IRIS+GRADIENT/igrad
        if "IrisMask" in Data:
            iMask = CreateShaderNodeTexImage(
                CurMat, self.BasePath + Data["IrisMask"], -900, 200, "Iris Mask", self.image_format
            )
        ...
        if "IrisColorGradient" in Data:
            ...
            for Entry in profile["gradientEntries"]:
                if counter == 0:
                    ...
                    colr = Entry["color"]
                    igradNode.color_ramp.elements[0].color = (
                        float(colr["Red"]) / 255,
