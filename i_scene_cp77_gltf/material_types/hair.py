...
class Hair:
    ...
    def create(self, hair, Mat):
        ...
        if not valid_json:
            self.report(
                {"ERROR"},
                "Incompatible hair profile json file detected. This add-on version requires materials generated WolvenKit 8.9.1 or higher.",
            )
            return
        ...
        aImg = imageFromRelPath(
            hair["Strand_Alpha"], DepotPath=self.BasePath, ProjPath=self.ProjPath, image_format=self.image_format
        )
        ...
        gImg = imageFromRelPath(
            hair["Strand_Gradient"], DepotPath=self.BasePath, ProjPath=self.ProjPath, image_format=self.image_format
        )
        ...
        for Entry in profile["gradientEntriesRootToTip"]:
            if counter == 0:
                ...
                colr = Entry["color"]
                RootToTip.color_ramp.elements[0].color = (
                    float(colr["Red"]) / 255,
        ...
        for Entry in profile["gradientEntriesID"]:
            if counter is 0:
                ...
                colr = Entry["color"]
                ID.color_ramp.elements[0].color = (
                    float(colr["Red"]) / 255,
                    float(colr["Green"]) / 255,
                    float(colr["Blue"]) / 255,
                    float(1),
