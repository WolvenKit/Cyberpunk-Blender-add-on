    ...
    def create(self, Data, Mat):
        ...
        if "TintColorMask" in Data:  # should always be param has a value in the skin.mt
            tImg = imageFromRelPath(
                Data["TintColorMask"], DepotPath=self.BasePath, ProjPath=self.ProjPath, isNormal=True
            )
        ...
        if "DetailNormalInfluence" in Data:
            nDNInfluence = CreateShaderNodeValue(
                CurMat, Data["DetailNormalInfluence"], -1250, -200, "DetailNormalInfluence"
            )
        ...
        if "MicroDetail" in Data:
            ...
            mdMapA = create_node(
                CurMat.nodes, "ShaderNodeTexImage", (-1100, -450), label="MicroDetail", image=mdMapAImg
            )
            mdMapB = create_node(
                CurMat.nodes, "ShaderNodeTexImage", (-1100, -650), label="MicroDetail", image=mdMapAImg
            )

        if "MicroDetailUVScale01" in Data:
            mdScale01 = CreateShaderNodeValue(
                CurMat, Data["MicroDetailUVScale01"], -2000, -1000, "MicroDetailUVScale01"
            )

        if "MicroDetailUVScale02" in Data:
            mdScale02 = CreateShaderNodeValue(
                CurMat, Data["MicroDetailUVScale02"], -2000, -1150, "MicroDetailUVScale02"
            )
        ...
        if "Detailmap_Squash" in Data:
            ...
            ndSqImgNode = create_node(
                CurMat.nodes, "ShaderNodeTexImage", (-2000, 50), label="Detailmap_Squash", image=sqshImg
            )
        ...
        if "Detailmap_Stretch" in Data:
            ...
            ndStImg = create_node(
                CurMat.nodes, "ShaderNodeTexImage", (-2000, 0), label="Detailmap_Stretch", image=strchImg
            )
