...
class MultilayeredClearCoat:
    ...
    def createBaseMaterial(self, matTemplateObj, name):
        ...
        NT = imageFromPath(
            self.BasePath + matTemplateObj["normalTexture"]["DepotPath"]["$value"], self.image_format, isNormal=True
        )
        RT = imageFromPath(
            self.BasePath + matTemplateObj["roughnessTexture"]["DepotPath"]["$value"], self.image_format, isNormal=True
        )
        MT = imageFromPath(
            self.BasePath + matTemplateObj["metalnessTexture"]["DepotPath"]["$value"], self.image_format, isNormal=True
        )
    ...
    def setGlobNormal(self, normalimgpath, CurMat, input):
        ...
        GNMap = CreateShaderNodeNormalMap(
            CurMat, self.BasePath + normalimgpath, -600, -550, "GlobalNormal", self.image_format
        )
    ...
    def createLayerMaterial(self, LayerName, LayerCount, CurMat, mlmaskpath, normalimgpath):
        for x in range(LayerCount - 1):
            MaskTexture = imageFromPath(
                os.path.splitext(self.BasePath + mlmaskpath)[0] + "_" + str(x + 1) + ".png",
                self.image_format,
                isNormal=True,
            )
            ...
            CurMat.links.new(
                CurMat.nodes["Mat_Mod_Layer_" + str(x + 1)].outputs[4], CurMat.nodes["Layer_" + str(x)].inputs[8]
            )
        ...
        CurMat.links.new(
            CurMat.nodes["Layer_" + str(LayerCount - 2)].outputs[0],
            CurMat.nodes["Principled BSDF"].inputs["Base Color"],
        )
        if normalimgpath:
            ...
            CurMat.links.new(
                CurMat.nodes["Layer_" + str(LayerCount - 2)].outputs[3],
                CurMat.nodes["Principled BSDF"].inputs["Normal"],
            )
        CurMat.links.new(
            CurMat.nodes["Layer_" + str(LayerCount - 2)].outputs[2], CurMat.nodes["Principled BSDF"].inputs["Roughness"]
        )
        CurMat.links.new(
            CurMat.nodes["Layer_" + str(LayerCount - 2)].outputs[1], CurMat.nodes["Principled BSDF"].inputs["Metallic"]
        )
        return
    ...
    def create(self, Data, Mat):
        ...
        if not valid_json:
            self.report(
                {"ERROR"},
                "Incompatible mlsetup json file detected. This add-on version requires materials generated WolvenKit 8.9.1 or higher.",
            )
            return
        ...
        for x in xllay:
            ...
            if not valid_json:
                self.report(
                    {"ERROR"},
                    "Incompatible mltemplate json file detected. This add-on version requires materials generated WolvenKit 8.9.1 or higher.",
                )
                return
            ...
            NG = bpy.data.node_groups.new(
                os.path.basename(Data["MultilayerSetup"])[:-8] + "_Layer_" + str(LayerIndex), "ShaderNodeTree"
