    ...
    def create(self, Data, Mat):
        ...
        if "Metalness" in Data:
            mImg = imageFromRelPath(
                Data["Metalness"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath, isNormal=True
            )
        ...
        if "Roughness" in Data:
            ...
            invr = create_node(CurMat.nodes, "ShaderNodeInvert", (-280.0, 90.0))
        ...
        Clamp2 = create_node(CurMat.nodes, "ShaderNodeClamp", (-538.0, 476.0))
        ...
        EnableMask = create_node(CurMat.nodes, "ShaderNodeValue", (-800.0, -800.0), label="EnableMask")
        EnableMask.outputs[0].default_value = int(self.enableMask)

        Math = create_node(CurMat.nodes, "ShaderNodeMath", (-500.0, -750.0), operation="SUBTRACT", label="Math")
        Math.inputs[0].default_value = 1

        Clamp001 = create_node(CurMat.nodes, "ShaderNodeClamp", (-200.0, -450.0), label="Clamp.001")

        CurMat.links.new(EnableMask.outputs["Value"], Math.inputs[1])  # Enablemask value into math which inverts it
        CurMat.links.new(
            Math.outputs["Value"], Clamp001.inputs[1]
        )  # Inverted value into clamp min, so if 1 its always solic, if 0 will use BaseColor alpha
        CurMat.links.new(
            bColNode.outputs["Alpha"], Clamp001.inputs[0]
        )  # alpha into one thats clamped by enableMask value
