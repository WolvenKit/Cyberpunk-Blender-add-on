...
class VehicleDestrBlendshape:
    ...
    def createLayerMaterial(self, LayerName, LayerCount, CurMat, mlmaskpath, normalimgpath):
        ...
        if LayerCount > 1:
            ...
            if normalimgpath:
                yoink = self.setGlobNormal(
                    normalimgpath, CurMat, CurMat.nodes["Layer_" + str(LayerCount - 2)].outputs[3]
                )
            ...
            CurMat.links.new(
                CurMat.nodes["Layer_" + str(LayerCount - 2)].outputs[2],
                CurMat.nodes["Principled BSDF"].inputs["Roughness"],
            )
            CurMat.links.new(
                CurMat.nodes["Layer_" + str(LayerCount - 2)].outputs[1],
                CurMat.nodes["Principled BSDF"].inputs["Metallic"],
            )
        return
