    ...
    def create(self, Data, Mat):
        ...
        if "DetailColor" in Data:
            dColImg = imageFromRelPath(
                Data["DetailColor"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath
            )
        ...
        if "Metalness" in Data:
            mImg = imageFromRelPath(
                Data["Metalness"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath
            )
            metNode = create_node(CurMat.nodes, "ShaderNodeTexImage", (-900, -100), label="Metalness", image=mImg)

        if "Roughness" in Data:
            rImg = imageFromRelPath(
                Data["Roughness"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath
            )
