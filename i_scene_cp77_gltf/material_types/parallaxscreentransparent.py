...
class ParallaxScreenTransparent:
    ...
    def create(self, Data, Mat):
        ...
        if "SeparateLayersFromTexture" in Data:
            separateLayersFromTex = CreateShaderNodeValue(
                CurMat, Data["SeparateLayersFromTexture"], -2000, 500, "SeparateLayersFromTexture"
            )
        ...
        if "LayersScrollSpeed" in Data:
            layersScrollSpeed_x = CreateShaderNodeValue(
                CurMat, Data["LayersScrollSpeed"]["X"], -2000, 450, "LayersScrollSpeed.x"
            )
            layersScrollSpeed_y = CreateShaderNodeValue(
                CurMat, Data["LayersScrollSpeed"]["Y"], -2000, 500, "LayersScrollSpeed.y"
            )
            layersScrollSpeed_z = CreateShaderNodeValue(
                CurMat, Data["LayersScrollSpeed"]["Z"], -2000, 550, "LayersScrollSpeed.z"
            )
            layersScrollSpeed_w = CreateShaderNodeValue(
                CurMat, Data["LayersScrollSpeed"]["W"], -2000, 600, "LayersScrollSpeed.w"
            )
        ...
        if "ScrollStepFactor1" in Data:
            scrollStepFactor1 = CreateShaderNodeValue(
                CurMat, Data["ScrollStepFactor1"], -2000, -400, "ScrollStepFactor1"
            )

        if "ScrollMaskHeight1" in Data:
            scrollMaskHeight1 = CreateShaderNodeValue(
                CurMat, Data["ScrollMaskHeight1"], -2000, -450, "ScrollMaskHeight1"
            )

        if "ScrollMaskStartPoint1" in Data:
            scrollMaskStartPoint1 = CreateShaderNodeValue(
                CurMat, Data["ScrollMaskStartPoint1"], -2000, -500, "ScrollMaskStartPoint1"
            )
        ...
        if "ScrollStepFactor2" in Data:
            scrollStepFactor2 = CreateShaderNodeValue(
                CurMat, Data["ScrollStepFactor2"], -2000, -600, "ScrollStepFactor2"
            )

        if "ScrollMaskHeight2" in Data:
            scrollMaskHeight2 = CreateShaderNodeValue(
                CurMat, Data["ScrollMaskHeight2"], -2000, -650, "ScrollMaskHeight2"
            )

        if "ScrollMaskStartPoint2" in Data:
            scrollMaskStartPoint2 = CreateShaderNodeValue(
                CurMat, Data["ScrollMaskStartPoint2"], -2000, -700, "ScrollMaskStartPoint2"
            )
        ...
        if "IntensityPerLayer" in Data:
            intensityPerLayer_x = CreateShaderNodeValue(
                CurMat, Data["IntensityPerLayer"]["X"], -2000, -800, "IntensityPerLayer.x"
            )
            intensityPerLayer_y = CreateShaderNodeValue(
                CurMat, Data["IntensityPerLayer"]["Y"], -2000, -850, "IntensityPerLayer.y"
            )
            intensityPerLayer_z = CreateShaderNodeValue(
                CurMat, Data["IntensityPerLayer"]["Z"], -2000, -900, "IntensityPerLayer.z"
            )
            intensityPerLayer_w = CreateShaderNodeValue(
                CurMat, Data["IntensityPerLayer"]["W"], -2000, -950, "IntensityPerLayer.w"
            )

        if "ScanlinesIntensity" in Data:
            scanlinesIntensity = CreateShaderNodeValue(
                CurMat, Data["ScanlinesIntensity"], -2000, -1000, "ScanlinesIntensity"
            )
        ...
        if "newUV_ps_t" in bpy.data.node_groups.keys():
            ...
            vecAdd2.inputs[1].default_value = (0.5, 0.5, 0.5)
        ...
        if "l1_ps_t" in bpy.data.node_groups.keys():
            ...
            vecMul3.inputs[1].default_value = (0.5, 0.5, 0)
        ...
        if "l2_ps_t" in bpy.data.node_groups.keys():
            ...
            vecMul8.inputs[1].default_value = (0.5, 0.5, 0)
            vecAdd6 = create_node(l2Group.nodes, "ShaderNodeVectorMath", (-600, 0), operation="ADD")
            vecAdd6.inputs[1].default_value = (0.5, 0, 0)
        ...
        if "l3" in bpy.data.node_groups.keys():
            ...
            clamp4.inputs[2].default_value = 0.5
            clamp5 = create_node(l3Group.nodes, "ShaderNodeClamp", (-150, -25))
            clamp5.inputs[1].default_value = 0.5
        ...
        scrollMask = create_node(
            CurMat.nodes, "ShaderNodeTexImage", (-950, 100), label="ScrollMaskTexture", image=scrollMaskImg
        )
        ...
        if "finalScrollUV2" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                finalScrollUV2Group.interface.new_socket(
                    name="finalScrollUV1", socket_type="NodeSocketVector", in_out="INPUT"
                )
                ...
                finalScrollUV2Group.interface.new_socket(
                    name="finalScrollUV2", socket_type="NodeSocketVector", in_out="OUTPUT"
                )
        ...
        if "finalScrollUV3" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                finalScrollUV3Group.interface.new_socket(
                    name="finalScrollUV1", socket_type="NodeSocketVector", in_out="INPUT"
                )
                ...
                finalScrollUV3Group.interface.new_socket(
                    name="finalScrollUV3", socket_type="NodeSocketVector", in_out="OUTPUT"
                )
        ...
        if "finalScrollUV4" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                finalScrollUV4Group.interface.new_socket(
                    name="finalScrollUV1", socket_type="NodeSocketVector", in_out="INPUT"
                )
                ...
                finalScrollUV4Group.interface.new_socket(
                    name="finalScrollUV4", socket_type="NodeSocketVector", in_out="OUTPUT"
                )
