    ...
    def create(self,Data,Mat):
        ...
        for param in Data:
            ...
            if isinstance(Data[param],str) and Data[param][-3:]=='xbm':
                ...
                textNode = create_node(CurMat.nodes,"ShaderNodeTexImage",  (imgx,imgy), label=param, image=text, hide=False)
                imgy=imgy-imgydelta
                if VERBOSE:
                    print("\ttext=imageFromRelPath(Data[param],self.image_format,DepotPath=self.BasePath, self.ProjPath=ProjPath)")
                    print("\ttextNode = create_node(CurMat.nodes,'ShaderNodeTexImage',  (x,y), label=param, image=text)")
                ...
                if 'Red' in Data[param]:
                    ...
                    if VERBOSE:
                        print("\tvector=create_node(CurMat.nodes,'ShaderNodeMapping',  (x,y), label=param)")
