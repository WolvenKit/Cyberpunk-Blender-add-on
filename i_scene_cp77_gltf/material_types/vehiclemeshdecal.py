...
class VehicleMeshDecal:
    ...
    def create(self, Data, Mat):
        ...
        if "DiffuseTexture" in Data:
            dcolImg = imageFromRelPath(
                Data["DiffuseTexture"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath
            )
            dImgNode = create_node(
                CurMat.nodes, "ShaderNodeTexImage", (-800, -500), label="DiffuseTexture", image=dcolImg
            )
        ...
        if "RoughnessTexture" in Data:
            rImg = imageFromRelPath(
                Data["RoughnessTexture"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath
            )
            rImgNode = create_node(
                CurMat.nodes, "ShaderNodeTexImage", (-800, -100), label="RoughnessTexture", image=rImg
            )
        ...
        if "MetalnessTexture" in Data:
            mImg = imageFromRelPath(
                Data["MetalnessTexture"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath
            )
        ...
        if "RoughnessMetalnessAlpha" in Data:
            rmAlphaVal = CreateShaderNodeValue(
                CurMat, Data["RoughnessMetalnessAlpha"], -1200, -50, "RoughnessMetalnessAlpha"
            )
