    ...
    def create(self, Data, Mat):
        ...
        if "RoomAtlas" in Data:
            bcolImg = imageFromRelPath(
                Data["RoomAtlas"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath
            )
        ...
        # Randomise Rooms
        WhiteNoiseTexture = create_node(
            CurMat.nodes,
            "ShaderNodeTexWhiteNoise",
            (-1077.558349609375, 946.2904052734375),
            label="White Noise Texture",
        )
        WhiteNoiseTexture.noise_dimensions = "1D"
        SeparateXYZ = create_node(
            CurMat.nodes, "ShaderNodeSeparateXYZ", (-1434.1156005859375, 899.5537719726562), label="Separate XYZ"
        )
        Math = create_node(
            CurMat.nodes, "ShaderNodeMath", (-1245.8779296875, 950.3339233398438), operation="FLOOR", label="Math"
        )
        Mathb = create_node(
            CurMat.nodes, "ShaderNodeMath", (-1245.8779296875, 930.3339233398438), operation="FLOOR", label="Mathb"
        )
        Mathc = create_node(
            CurMat.nodes, "ShaderNodeMath", (-1205.8779296875, 930.3339233398438), operation="MULTIPLY", label="Mathc"
        )
        Math002 = create_node(
            CurMat.nodes,
            "ShaderNodeMath",
            (-891.7330322265625, 936.3861694335938),
            operation="MULTIPLY",
            label="Math.002",
        )
        Math002.inputs[1].default_value = 5
        Math001 = create_node(
            CurMat.nodes, "ShaderNodeMath", (-715.7330322265625, 918.6345825195312), operation="FLOOR", label="Math.001"
        )

        # Window properties
        if "WindowTexture" in Data:
            wcolImg = imageFromRelPath(
                Data["WindowTexture"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath
            )
            wColNode = create_node(
                CurMat.nodes, "ShaderNodeTexImage", (-600, -150), label="WindowTexture", image=wcolImg
            )
            if "Normal" in Data:
                wNormImg = imageFromRelPath(
                    Data["Normal"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath, isNormal=True
                )
                wNormNode = create_node(
                    CurMat.nodes, "ShaderNodeTexImage", (-600, -200), label="Normal", image=wNormImg
                )
            if "NormalStrength" in Data:
                NormalStrength = CreateShaderNodeValue(CurMat, Data["NormalStrength"], -600, -250, "NormalStrength")
            if "Roughness" in Data:
                wRoughImg = imageFromRelPath(
                    Data["Roughness"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath, isNormal=True
                )
                wRoughNode = create_node(
                    CurMat.nodes, "ShaderNodeTexImage", (-600, -300), label="Roughness", image=wRoughImg
                )
