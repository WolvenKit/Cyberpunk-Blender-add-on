    ...
    def create(self, Data, Mat):
        ...
        if "DiffuseTexture" in Data:
            dImg = imageFromRelPath(
                Data["DiffuseTexture"], DepotPath=self.BasePath, ProjPath=self.ProjPath, image_format=self.img_format
            )
        ...
        if "NormalTexture" in Data:
            nMap = CreateShaderNodeNormalMap(
                CurMat, self.BasePath + Data["NormalTexture"], -200, -250, "NormalTexture", self.img_format
            )
        ...
        if "NormalAlphaTex" in Data:
            nAImg = imageFromRelPath(
                Data["NormalAlphaTex"], DepotPath=self.BasePath, ProjPath=self.ProjPath, image_format=self.img_format
            )
        ...
        if "RoughnessTexture" in Data:
            rImg = imageFromRelPath(
                Data["RoughnessTexture"], DepotPath=self.BasePath, ProjPath=self.ProjPath, image_format=self.img_format
            )
        ...
        if "MetalnessTexture" in Data:
            mImg = imageFromRelPath(
                Data["MetalnessTexture"], DepotPath=self.BasePath, ProjPath=self.ProjPath, image_format=self.img_format
            )
