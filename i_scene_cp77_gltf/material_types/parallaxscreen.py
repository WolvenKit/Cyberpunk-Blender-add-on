...
class ParallaxScreen:
    ...
    def createStepGroup(self):
        if "step" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                stepGroup.interface.new_socket(name="y", socket_type="NodeSocketFloat", in_out="INPUT")
                stepGroup.interface.new_socket(name="x", socket_type="NodeSocketFloat", in_out="INPUT")
                stepGroup.interface.new_socket(name="result", socket_type="NodeSocketFloat", in_out="OUTPUT")
    ...
    def colorlessTexGroup(self):
        if "colorlessTex" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                colorlessTexGroup.interface.new_socket(name="Color", socket_type="NodeSocketColor", in_out="INPUT")
                colorlessTexGroup.interface.new_socket(name="Color", socket_type="NodeSocketColor", in_out="OUTPUT")
    ...
    def create(self, Data, Mat):
        ...
        if "ScanlinesIntensity" in Data:
            scanlinesIntensity = CreateShaderNodeValue(
                CurMat, Data["ScanlinesIntensity"], -2000, 0, "ScanlinesIntensity"
            )

        if "IntensityPerLayer" in Data:
            intensityPerLayer_x = CreateShaderNodeValue(
                CurMat, Data["IntensityPerLayer"]["X"], -2000, -50, "IntensityPerLayer.x"
            )
            intensityPerLayer_y = CreateShaderNodeValue(
                CurMat, Data["IntensityPerLayer"]["Y"], -2000, -100, "IntensityPerLayer.y"
            )
            intensityPerLayer_z = CreateShaderNodeValue(
                CurMat, Data["IntensityPerLayer"]["Z"], -2000, -150, "IntensityPerLayer.z"
            )
        ...
        if "ScrollStepFactor1" in Data:
            scrollStepFactor1 = CreateShaderNodeValue(
                CurMat, Data["ScrollStepFactor1"], -2000, -300, "ScrollStepFactor1"
            )

        if "ScrollMaskHeight1" in Data:
            scrollMaskHeight1 = CreateShaderNodeValue(
                CurMat, Data["ScrollMaskHeight1"], -2000, -350, "ScrollMaskHeight1"
            )

        if "ScrollMaskStartPoint1" in Data:
            scrollMaskStartPoint1 = CreateShaderNodeValue(
                CurMat, Data["ScrollMaskStartPoint1"], -2000, -400, "ScrollMaskStartPoint1"
            )
        ...
        if "ScrollStepFactor2" in Data:
            scrollStepFactor2 = CreateShaderNodeValue(
                CurMat, Data["ScrollStepFactor2"], -2000, -550, "ScrollStepFactor2"
            )

        if "ScrollMaskHeight2" in Data:
            scrollMaskHeight2 = CreateShaderNodeValue(
                CurMat, Data["ScrollMaskHeight2"], -2000, -600, "ScrollMaskHeight2"
            )

        if "ScrollMaskStartPoint2" in Data:
            scrollMaskStartPoint2 = CreateShaderNodeValue(
                CurMat, Data["ScrollMaskStartPoint2"], -2000, -650, "ScrollMaskStartPoint2"
            )
        ...
        if "ScrollVerticalOrHorizontal" in Data:
            scrollVerticalOrHorizontal = CreateShaderNodeValue(
                CurMat, Data["ScrollVerticalOrHorizontal"], -2000, -750, "ScrollVerticalOrHorizontal"
            )
        ...
        if "ParalaxTexture" in Data:
            parImg = imageFromRelPath(
                Data["ParalaxTexture"], DepotPath=self.BasePath, ProjPath=self.ProjPath, image_format=self.image_format
            )

        if "BlinkingMaskTexture" in Data:
            blinkImg = imageFromRelPath(
                Data["BlinkingMaskTexture"],
        ...
        if "ScanlineTexture" in Data:
            scanLineImg = imageFromRelPath(
                Data["ScanlineTexture"], DepotPath=self.BasePath, ProjPath=self.ProjPath, image_format=self.image_format
            )

        if "ScrollMaskTexture" in Data:
            scrollMaskImg = imageFromRelPath(
                Data["ScrollMaskTexture"],
                DepotPath=self.BasePath,
                ProjPath=self.ProjPath,
                image_format=self.image_format,
            )
        ...
        vecSub.inputs[1].default_value = (0.5, 0.5, 0.5)
        ...
        vecAdd.inputs[1].default_value = (0.5, 0.5, 0.5)
        ...
        if "scroll1" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                scroll1Group_inputs = scroll1Group.inputs
            else:
                scroll1Group.interface.new_socket(name="ScrollSpeed1", socket_type="NodeSocketFloat", in_out="INPUT")
                scroll1Group.interface.new_socket(
                    name="ScrollStepFactor1", socket_type="NodeSocketFloat", in_out="INPUT"
                )
        ...
        if "scrollUV1" in bpy.data.node_groups.keys():
            ...
            scrollUV1Group_inputs = get_inputs(scrollUV1Group)
        else:
            ...
            if vers[0] < 4:
                ...
                scrollUV1Group_inputs = scrollUV1Group.inputs
            else:
                scrollUV1Group.interface.new_socket(name="newUV", socket_type="NodeSocketVector", in_out="INPUT")
                scrollUV1Group.interface.new_socket(
                    name="ScrollMaskHeight1", socket_type="NodeSocketFloat", in_out="INPUT"
                )
                scrollUV1Group.interface.new_socket(name="scroll1", socket_type="NodeSocketFloat", in_out="INPUT")
                scrollUV1Group.interface.new_socket(
                    name="ScrollMaskStartPoint1", socket_type="NodeSocketFloat", in_out="INPUT"
                )
        ...
        CurMat.links.new(vecAdd.outputs[0], scrollUV1.inputs["newUV"])
        CurMat.links.new(scrollMaskHeight1.outputs[0], scrollUV1.inputs["ScrollMaskHeight1"])
        CurMat.links.new(scroll1.outputs[0], scrollUV1.inputs["scroll1"])
        CurMat.links.new(scrollMaskStartPoint1.outputs[0], scrollUV1.inputs["ScrollMaskStartPoint1"])
        # scrollUV1X
        if "scrollUV1X" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                scrollUV1XGroup.interface.new_socket(
                    name="ScrollMaskHeight1", socket_type="NodeSocketFloat", in_out="INPUT"
                )
        ...
        if "scroll2" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                scroll2Group.interface.new_socket(
                    name="ScrollStepFactor2", socket_type="NodeSocketFloat", in_out="INPUT"
                )
        ...
        if "scrollUV2" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                scrollUV2Group.interface.new_socket(
                    name="ScrollMaskHeight2", socket_type="NodeSocketFloat", in_out="INPUT"
                )
                scrollUV2Group.interface.new_socket(name="scroll2", socket_type="NodeSocketFloat", in_out="INPUT")
                scrollUV2Group.interface.new_socket(
                    name="ScrollMaskStartPoint2", socket_type="NodeSocketFloat", in_out="INPUT"
                )
        ...
        if "scrollUV2X" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                scrollUV2XGroup.interface.new_socket(
                    name="ScrollMaskHeight2", socket_type="NodeSocketFloat", in_out="INPUT"
                )
        ...
        if "rndColorIndex" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                rndColorIGroup.interface.new_socket(name="brokenUV", socket_type="NodeSocketVector", in_out="INPUT")
                rndColorIGroup.interface.new_socket(
                    name="rndColorIndex", socket_type="NodeSocketFloat", in_out="OUTPUT"
                )
        ...
        if "rndColor" in bpy.data.node_groups.keys():
            ...
            combine6.inputs[0].default_value = 0.2
        ...
        if "rndOff" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                rndOffGroup.interface.new_socket(name="x", socket_type="NodeSocketFloat", in_out="OUTPUT")
                rndOffGroup.interface.new_socket(name="y", socket_type="NodeSocketFloat", in_out="OUTPUT")
                rndOffGroup.interface.new_socket(name="z", socket_type="NodeSocketFloat", in_out="OUTPUT")
                rndOffGroup.interface.new_socket(name="w", socket_type="NodeSocketFloat", in_out="OUTPUT")

            rndOffGroupO = create_node(rndOffGroup.nodes, "NodeGroupOutput", (0, 0))
            rndOffGroupO.inputs[0].default_value = -0.01
            rndOffGroupO.inputs[1].default_value = 0.015
            rndOffGroupO.inputs[2].default_value = -0.02
            rndOffGroupO.inputs[3].default_value = 0.01
        ...
        if "randomOffset" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                randomOffsetGroup.interface.new_socket(name="rand", socket_type="NodeSocketFloat", in_out="INPUT")
                randomOffsetGroup.interface.new_socket(name="rndOff.x", socket_type="NodeSocketFloat", in_out="INPUT")
                randomOffsetGroup.interface.new_socket(name="rndOff.y", socket_type="NodeSocketFloat", in_out="INPUT")
                randomOffsetGroup.interface.new_socket(name="rndOff.z", socket_type="NodeSocketFloat", in_out="INPUT")
                randomOffsetGroup.interface.new_socket(name="rndOff.w", socket_type="NodeSocketFloat", in_out="INPUT")
                randomOffsetGroup.interface.new_socket(
                    name="randomOffset", socket_type="NodeSocketVector", in_out="OUTPUT"
                )
        ...
        if "newRandomOffset" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                newRandomOffsetGroup.interface.new_socket(name="rand", socket_type="NodeSocketFloat", in_out="INPUT")
                newRandomOffsetGroup.interface.new_socket(
                    name="randomOffset", socket_type="NodeSocketVector", in_out="INPUT"
                )
                newRandomOffsetGroup.interface.new_socket(
                    name="newRandomOffset", socket_type="NodeSocketVector", in_out="OUTPUT"
                )
            ...
            step.inputs[0].default_value = 0.7
            ...
            vecLerp.inputs[0].default_value = (0.01, -0.01, 0)
        ...
        if "l2" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                l2Group.interface.new_socket(name="l2", socket_type="NodeSocketFloat", in_out="INPUT")
                l2Group.interface.new_socket(name="newRandomOffset1", socket_type="NodeSocketVector", in_out="OUTPUT")
        ...
        if "if BlinkingSpeed > 0" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                bl1Group.interface.new_socket(name="BlinkingSpeed", socket_type="NodeSocketFloat", in_out="INPUT")
                bl1Group.interface.new_socket(name="time", socket_type="NodeSocketFloat", in_out="INPUT")
                bl1Group.interface.new_socket(name="iA", socket_type="NodeSocketColor", in_out="INPUT")
                bl1Group.interface.new_socket(name="i1", socket_type="NodeSocketColor", in_out="INPUT")
                bl1Group.interface.new_socket(name="new_i1", socket_type="NodeSocketColor", in_out="OUTPUT")
        ...
        if "scanlineUV" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                scanlineUVGroup.interface.new_socket(
                    name="ScanlinesDensity", socket_type="NodeSocketFloat", in_out="INPUT"
                )
                scanlineUVGroup.interface.new_socket(name="time", socket_type="NodeSocketFloat", in_out="INPUT")
                scanlineUVGroup.interface.new_socket(name="UV", socket_type="NodeSocketVector", in_out="INPUT")
                scanlineUVGroup.interface.new_socket(name="scanlineUV", socket_type="NodeSocketVector", in_out="OUTPUT")
        ...
        # sampledScanlines
        scanlineTexture = create_node(
            CurMat.nodes, "ShaderNodeTexImage", (-800, 100), label="ScanlineTexture", image=scanLineImg
        )
        ...
        if "m2" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                m2Group.interface.new_socket(name="i3", socket_type="NodeSocketVector", in_out="INPUT")
        ...
        greaterThan2.inputs[1].default_value = 0.28
        lessThan2 = create_node(CurMat.nodes, "ShaderNodeMath", (-950, -275), operation="LESS_THAN")
        lessThan2.inputs[1].default_value = 0.543
        ...
        greaterThan3.inputs[1].default_value = 0.78
        lessThan3 = create_node(CurMat.nodes, "ShaderNodeMath", (-950, -375), operation="LESS_THAN")
        lessThan3.inputs[1].default_value = 0.85
        ...
        vecMul21.inputs[0].default_value = (0.4, 0.4, 0.4)
        ...
        scrollMask = create_node(
            CurMat.nodes, "ShaderNodeTexImage", (-1700, -650), label="ScrollMaskTexture", image=scrollMaskImg
        )
        ...
        if "finalScrollUV" in bpy.data.node_groups.keys():
            ...
            if vers[0] < 4:
                ...
                finalScrollUVGroup.interface.new_socket(
                    name="finalScrollUV", socket_type="NodeSocketVector", in_out="OUTPUT"
                )
        ...
        mul12.inputs[1].default_value = 0.75
        ...
        combine22.inputs[0].default_value = 0.03
        combine22.inputs[1].default_value = 0.03
        combine22.inputs[2].default_value = 0.03
